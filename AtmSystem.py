# -*- coding: utf-8 -*-
"""
Created on Sat Oct 22 11:45:37 2022
...
def Submit():
    ...
    if cnum1=="12345" and pin1=="234":
        ...
        messagebox.showinfo('Info',f'Welcome Mam/Sir to our ATM')
        win=tk.Tk()
...
#BALANCE########################################################################################################################          
        ...
        def billpay1():
            ...
            def paym():
                ...
                messagebox.showinfo('Bill Paid',f'You have successfully paid your bill {paymt}')
            ...
            e3=Entry(bill,width=30, borderwidth=2, relief='sunken', font=('Gentium Plus',10))
            e3.place(x=410,y=200)
                
            b1=Button(bill,text='Pay Amount',bg='red',fg='white',activebackground='black',width=13,height=3, relief='solid',command=paym,                 
                      font=('Calibiri',13))
...
#DATABASE--------------------------------------------------------------------------------------------------------------------
            ...
            lst1=[bal1,paid,depm,wd]
    
            conn=s.connect('AtmSystem.db')
            with conn:
              cursor=conn.cursor()
            o=s.connect('AtmSystem.db')
            if o:
              print("Database is created and connected")
            cur=o.cursor()
            cur.execute("create table if not exists AtmBalanceEntry(Balance text,Bill Pay text,Deposit text,Withdrawn text) ")
            cur.execute("insert into AtmBalanceEntry Values(?,?,?,?)",lst1)
            o.commit()
            o.close()
...
#WITHDRAWL#############################################################################################################################
        ...
        def withdraw():
            ...
            b1=Button(wit,text='Withdraw Amount',bg='red',fg='white',activebackground='black',width=20,height=3, relief='solid',
                      command=wtd,font=('Calibiri',13))
...
#DEPOSIT#####################################################################################################################
        ...
        def depst():
            ...
            b1=Button(dep,text='Deposit Amount',bg='red',fg='white',activebackground='black',width=15,height=3, relief='solid',command=deps,               
                      font=('Calibiri',13))
            b1.place(x=320,y=300)
